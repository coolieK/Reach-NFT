...
const startBidders = async () => {
    ...
    const runBidder = async (who) => {
        ...
        const acc = await stdlib.newTestAccount(startBidders);
        acc.setDebugLabel(who);
        await acc.tokenAccept(nftId);
        bidders.push(who, acc);
        const ctc = acc.contract(backend, ctcCreator.getInfo());
        const getBal = async () = stdlib.formatCurrency(await stdlib.balancesOf(acc));
        ...
        console.log(`${who} failed balance after is ${await getBal()}`);
    };
...
await ctcCreator.participants.Creator({
    getSale: () => {
        console(`Creator sets parameters for sale: `,params);
        return params;
    },
    auctionReady: () => {
        startBidders();
